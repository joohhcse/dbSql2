...
--row_3 : emp 테이블에서 ename 컬럼 기준으로 오름차순 정렬 했을때 11~14번째 행의 데이터만
--        조회하는 sql을 작성하세요
...
--SINGLE FOW FUNCTION : 행당 한번의 FUNCTION이 실행
...
FROM emp
WHERE ename = UPPER('smith'); --직원 이름이 smith인 사람을 조회하려면
...
SELECT empno, LOWER(ename) low_enm
FROM emp
WHERE LOWER(ename) = 'smith';   --WHERE 절에 가공한 값을 넣으면 안됨
...
SELECT CONCAT(CONCAT('HELLO', ', '), 'WORLD') c1,
       ...
       SUBSTR('HELLO, WORLD', 1, 5) s1, --SUBSTR(문자열, 시작인덱스, 종료인덱스) --인덱스가 1부터, 종료인덱스도 포함
       
       --INSTR : 문자열에 특정 문자열이 존재하는지, 존재할 경우 문자의 인덱스를 리턴
       INSTR('HELLO, WORLD', 'O') i1,  --5, 9
       --'HELLO, WORLD' 문자열의 6번째 인덱스 이후에 등장하는 'O' 문자열의 인덱스 리턴
       INSTR('HELLO, WORLD', 'O', 6) i2,   --문자열의 특정 인덱스 이후부터 검색 하도록 옵션값
       
       INSTR('HELLO, WORLD', 'O', INSTR('HELLO, WORLD', 'O') + 1) i3,  --문자열의 특정 인덱스 이후부터 검색 하도록 옵션값
       
       --L/RPAD 특정 문자열의 왼쪽/오른쪽에 설정한 문자열 길이보다 부족한 만큼 문자열을
       --채워넣는다
       LPAD('HELLO, WORLD', 15, '*') L1,
       LPAD('HELLO, WORLD', 15 ) L2,   --DEFAULT 채움 문자는 공백
...
--TRUNC : 절삭 - ROUNC(숫자, 절삭 자리)
--MOD : 나머지 연산(피제수, 제수) --MOD(5,2) = 1
...
SELECT --절삭 결과가 소수점 한자리까지 나오도록(소수점 둘째자리에서 절삭)
    TRUNC(105.54, 1) t1, 
    TRUNC(105.55, 1) t2,
    TRUNC(105.55, 0) t3, --정수까지 출력 --소수점 첫번째 자리에서 반올림
    TRUNC(105.55, -1) t4, --정수 첫번째 자리에서 반올림
FROM dual;


--MOD(피제수, 제수) 피제수를 제수로 나눈 나머지값 --MOD : %
--MOD(M, 2)의 결과 종류 : 0, 1 ( 0~제수-1)
SELECT MOD(5, 2) M1 -- 5/2 = 몫이 2, [나머지가 1]
FROM dual;

--emp 테이블의 sal 컬럼을 1000으로 나눴을때 사원별 나머지 값을 조회하는 sql작성
--ename, sal, sal/1000 일때의 몫, sal/1000 일때의 나머지
...
SELECT TO_CHAR(SYSDATE, 'YYYY') YYYY, -- 현재 년도
    ...
    TO_CHAR(SYSDATE, 'IW') IW, --현재 일자의 주차(주간일자1~7)
