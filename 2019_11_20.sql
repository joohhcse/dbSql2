...
--날짜 + 정수 연산은 >> 일자를 더한 날짜타입이 결과로 나온다
...
-- null연산을 시험해 보기 위해 moon의 reg_dt 컬럼을 null로 변경
...
SELECT userid, usernm,
       ...
       CONCAT(userid, CONCAT(usernm, pass)) AS concat_id_name_pass
FROM users;
...
SELECT 'SELECT * FROM ' || table_name || ';' AS query,
        --CONCAT 함수만 이용해서
--        CONCAT(CONCAT('SELECT * FROM ', table_name), ';') AS query2
        -- 1. 'SELECT * FROM '
        -- 2. 'table_name
        -- 3. ';'
        CONCAT(CONCAT('SELECT * FROM ', table_name), ';') AS query3
FROM user_tables;
...
-- 사원 입사일자(hiredate) 1982년 1월 1일 이후인 사원 정보 조회
